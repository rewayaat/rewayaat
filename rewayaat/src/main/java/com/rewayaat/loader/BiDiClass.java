...
/**
 * This class is an implementation the the ICU4J class. TextNormalize will call
 * this only if the ICU4J library exists in the classpath.
 * 
...
public class BiDiClass {
	...
	public BiDiClass() {
		 ...
		 * Bidi.REORDER_INVERSE_NUMBERS_AS_L, which caused problems in some test
		 * files. For example, a file had a line of: 0 1 / ARABIC and the 0 and
		 * 1 were reversed in the end result. REORDER_INVERSE_LIKE_DIRECT is the
		 * inverse Bidi mode that more closely reflects the Unicode spec.
		 */
	...
	public String makeLineLogicalOrder(String str, boolean isRtlDominant) {
		...
		/*
		 * Set the mirror flag so that parentheses and other mirror symbols are
		 * properly reversed, when needed. With this removed, lines such as
		 * (CBA) in the PDF file will come out like )ABC( in logical order.
		 */
	...
	public String normalizeDiac(String str) {
		...
		{
			char c = str.charAt(i);
			if (Character.getType(c) == Character.NON_SPACING_MARK || Character.getType(c) == Character.MODIFIER_SYMBOL
