...
/**
 * Generates and backfills E5-large semantic embeddings for Quran verses.
 ...
 *
 * Usage: java -cp rewayaat.jar com.rewayaat.tools.QuranVerseEmbeddingTool
 */
public class QuranVerseEmbeddingTool {

    private static final ObjectMapper MAPPER = new ObjectMapper();
    private static final Duration REQUEST_TIMEOUT = Duration.ofMinutes(2);
    private static final String QURAN_INDEX = resolveQuranIndex();
    ...
    public static void main(String[] args) throws Exception {
        new QuranVerseEmbeddingTool().run();
    }
    ...
    private void processEmbeddings(List<DocEmbedding> docs) throws Exception {
        ...
        for (int from = 0; from < docs.size(); from += EMBEDDING_BATCH_SIZE) {
            ...
            try {
                ...
                System.err.println("Batch inference failed for " + batch.size() + " Quran verses: " + batchEx.getMessage());
    ...
    private void ensureSemanticVectorField() throws Exception {
        System.out.println("Checking for semantic_vector field in index: " + QURAN_INDEX);
        try {
            ...
            HttpRequest request = baseRequest("/" + encode(QURAN_INDEX) + "/_mapping")
            ...
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                System.out.println("Successfully added semantic_vector field to index: " + QURAN_INDEX);
            }
        } catch (Exception e) {
    ...
    private JsonNode startScroll() throws Exception {
        ...
        source.add("text_english");
        source.add("text_arabic");
        source.add(SEMANTIC_VECTOR_FIELD);
        body.set("query", MAPPER.createObjectNode().set("match_all", MAPPER.createObjectNode()));
        return postJson("/" + encode(QURAN_INDEX) + "/_search?scroll=" + encode(SCROLL_KEEPALIVE), body.toString());
    }
    ...
    private String extractEmbeddingText(JsonNode source) {
        String english = textValue(source, "text_english");
        String arabic = textValue(source, "text_arabic");
        if (!arabic.isBlank() && !english.isBlank()) {
            return arabic + "\n" + english;
        }
        if (!english.isBlank()) {
            return english;
        }
        return arabic.isBlank() ? null : arabic;
    }
    ...
    private static String resolveQuranIndex() {
        String index = System.getProperty("quran.index");
        if (index == null || index.isEmpty()) {
            index = System.getenv().get("QURAN_INDEX");
        }
        return (index != null && !index.isEmpty()) ? index : "rewayaat_quran";
    }
